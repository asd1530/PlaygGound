...
namespace Common.Sie
{
    public class SieDocumentComparer
    {
        ...
        private void compareKONTO(SieDocument docA, SieDocument docB, string nameA, string nameB)
        {
            foreach (var kA in docA.KONTO.Values)
            {
                if (docB.KONTO.ContainsKey(kA.Number))
                {
                    ...
                    if (kA.AccountType != kB.AccountType) _errors.Add("KONTO.Type differ " + kA.Number);
        ...
        private void compareRAR(SieDocument docA, SieDocument docB, string nameA, string nameB)
        {
            ...
            {
                if (docB.RAR.ContainsKey(rarA.FiscalYearId))
                {
                    var rarB = docB.RAR[rarA.FiscalYearId];
                    if (rarA.Start != rarB.Start) _errors.Add(nameB + "RAR differs " + rarA.FiscalYearId);
                    if (rarA.End != rarB.End) _errors.Add(nameB + "RAR differs " + rarA.FiscalYearId);
                }
                else
                {
                    _errors.Add(nameB + "RAR is missing " + rarA.FiscalYearId);
                }
            }
        }
